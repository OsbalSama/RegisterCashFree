...
public class allTypes extends javax.swing.JFrame {
     ...
     */
    String Class = "allTypes";
    typeContainerController dbTypes = typeContainerController.getInstance();
    ...
    public void loadFrame() {
        this.setTitle("Mostrar Categorias");
            ...
            tableData.addMouseListener(new MouseAdapter() {
                ...
                public void mousePressed(MouseEvent Mouse_evt) {
                    ...
                    if (Mouse_evt.getClickCount() == 2 && Mouse_evt.getButton() == MouseEvent.BUTTON1) {
                        try {
                            updateReg(dbTypes.findTypebyDesc(tableData.getValueAt(tableData.getSelectedRow(), 1).toString()));
                        } catch (Exception e) {
    ...
    public void updateReg(Group type) {
        try {
            addTextValue contDesc = new addTextValue(null, true);
            contDesc.setData("Ver Categoria", type.getDescription());
            contDesc.show();
            if (contDesc.isNext()) {
                if (contDesc.isDelete()) {
                    if (dbTypes.deleleType(type)) {
                        new Messages().closeMessage("Eliminar Categoria", "Categoria Eliminada...");
                    }
                } else {
                    type.setDescription(contDesc.getContent());
                    if (dbTypes.updateType(type)) {
                        new Messages().closeMessage("Actualizar Categoria", "Categoria Actualizada...");
                    }
                }
                showDatabase();
            }
        } catch (Exception e) {
            new Messages().errorMessage(Class, "updateReg()", e);
        }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Mostrar Categorias");
        ...
        btnNewProd.setText("Nueva Categoria");
    ...
    public void newReg() {
        try {
            ...
            regDesc.show();
            if (regDesc.isNext()) {
                if (!dbTypes.existType(regDesc.getContent())) {
                    Group newReg = new Group();
                    newReg.genId();
                    newReg.setDescription(regDesc.getContent());
                    if (dbTypes.createType(newReg)) {
                    ...
                    String text = "<html>"
                            ...
                            + "</center>"
                            + "</html>";
                    if (new Messages().yesNoMessage("Agregar Categoria", text)) {
                        newReg();
                    }
                }
            }
        } catch (Exception e) {
            new Messages().errorMessage(Class, "newReg()", e);
        }
    }
    ...
    public void showDatabase() {
        ...
        try {
            for (int i = 0; i < dbTypes.getTypes().size(); i++) {
                Group temp = dbTypes.getTypes().get(i);
